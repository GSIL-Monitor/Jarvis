...
public class Pro5_utorid {
	...
	public static void main(String[] args) throws IOException {
		...
		do {
			...
			if (choice.equalsIgnoreCase("M")) {
				...
				// check that they have all been solved with this com.mrliuxia.jarvis.model instance
	...
	public static SCPModel getModelData() {
		System.out.println();
		int n = Pro5_utorid.getInteger("Enter number of elements (n): ", 1, Integer.MAX_VALUE);
		int m = Pro5_utorid.getInteger("Enter number of sets (m): ", 1, Integer.MAX_VALUE);
		
		SCPModel scpmodel = new SCPModel();
		for (int i = 1; i <= m; i++) {
			System.out.println("\nSet " + i + " details");
			double cost = Pro5_utorid.getDouble("   Enter cost of set " + i + ": ", -Double.MAX_VALUE, Double.MAX_VALUE);
			
			int j = 1;
			List<Integer> elements = new ArrayList<Integer>();
			while(j != 0) {
				j = Pro5_utorid.getInteger("   Enter an element covered by set " + i + " (0 to stop): ", 0, n);
				if (j != 0) 
					elements.add(j);
			}
	...
	public static void displayMenu() {
		...
		System.out.println("M - Enter SCP com.mrliuxia.jarvis.model data");
