...
public abstract class GreedySolver {
	protected String _name;			  // name of com.mrliuxia.jarvis.algorithm type
	protected double _alpha;          // minimum required coverage level
	protected SCPModel _model;        // the SCP com.mrliuxia.jarvis.model we're currently operating on
	...
	protected boolean _solved;        // whether or not the com.mrliuxia.jarvis.model has been solved
	...
	public void solve() {
		...
		for (Integer element : _model.getElementIterable()) {
			_elementsNotCovered.add(element);
		}
		...
		while (_elementsNotCovered.size() > 0 &&
			...
			ElementSet nextBest = nextBestSet();

			// TODO: Update solnSets, objFn, elements_not_covered, and all_selected.
			_solnSets.add(nextBest);
			_objFn += nextBest.getCost();
			
			for (Integer i : nextBest.getElementIterable()) {
				_elementsNotCovered.remove(i);
			}

			all_selected = true;
			for (ElementSet set : _model.getElementSetIterable()) {
				if (_solnSets.contains(set)) {
					continue;
				}
				for (Integer id : set.getElementIterable()) {
					if (_elementsNotCovered.contains(id)) {
						all_selected = false;
					}
				}
			...
			System.out.println("- Selected: " + nextBest);
		}
	...
	// NOTE: if no set can improve the solution, returns null to allow the greedy com.mrliuxia.jarvis.algorithm to terminate.
